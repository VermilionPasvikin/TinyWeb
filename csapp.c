...
void Getnameinfo(const SA *sockaddr, socklen_t socklen,
                 char *host, size_t hostlen,
                 char *service, size_t servlen, int flags) 
{
    int rc;

    if ((rc = getnameinfo(sockaddr, socklen, host, hostlen, service, servlen, flags)) != 0)
    {
...
{
    ssize_t bytes_written = rio_writen(fd, usrbuf, n);
    if (bytes_written != n) {
	/* 如果是EPIPE错误，记录日志但不终止程序 */
	if (errno == EPIPE) {
	    fprintf(stderr, "EPIPE error: Client closed connection prematurely\n");
	    return; /* 优雅地返回而不终止程序 */
	} else {
	    unix_error("Rio_writen error"); /* 其他错误仍然终止程序 */
	}
    }
}
